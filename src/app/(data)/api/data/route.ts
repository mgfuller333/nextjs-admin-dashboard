...
export async function POST(req: NextRequest) {
  try {
   ...
   // console.log('Raw Payload:', JSON.stringify(payload, null, 2));
    ...
    // Basic check: ensure payload is an array
    if (!Array.isArray(payload)) {
      return NextResponse.json({ error: 'Payload must be an array' }, { status: 400 });
    }

    //console.log('Post-Processed Rows:', JSON.stringify(payload, null, 2));
    ...
    // Include payload and rows in response for client-side logging
    return NextResponse.json({
      message: 'Data successfully uploaded to BigQuery',
      rowCount: payload.length,
      payload: payload,
      processedRows: payload,
    }, { status: 200 });

  } catch (error) {
    console.error('Error uploading to BigQuery:', error);
    return NextResponse.json(
      { error: 'Failed to upload data to BigQuery', details: (error as Error).message },
      { status: 500 }
    );
  }
}
