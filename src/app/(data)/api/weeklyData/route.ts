...
export async function GET(req: NextRequest) {
  console.log('GET request received with params:', req.nextUrl.searchParams.toString()); // Debug: Confirm handler hit

  try {
    const { searchParams } = req.nextUrl;

    // Parse keys (handle multi or single)
    ...
    // Default: last 12 months (use actual current date for real-time data)
    if (!start || !end) {
      const now = new Date(); // Dynamic now (removes fixed test date for prod/debug)
      end = now.toISOString();
      start = new Date(now.getFullYear() - 1, now.getMonth(), now.getDate()).toISOString();
    }

    // Build SQL — Use WHERE for efficiency, keep LIMIT 100 for safety (adjust based on data volume)
    ...
    const [rows] = await bigquery.query(options); // Destructure properly: [rows] gets the row array
    ...
    rows.forEach((row: any) => {
      const timestamp = row.ts?.value || row.ts; // Safe access for TIMESTAMP format
      keys.forEach(key => {
        const value = row[key];
    ...
    console.log(`Fetched data: ${rows.length} rows from ${start} to ${end}`); // Uncommented & tweaked label (not "Weekly" since it's 12mo)
